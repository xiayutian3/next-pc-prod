...
function GuessVideo(props) {
  ...
  return (
    <div className="guess-video">
      ...
      <div className="msg">
        <div className="title">
          <Link className="link" href={`/playpage?videoSrc=${videoItem.videoSrc}`}>  {videoItem.title}</Link>
          </div>
        <div className="name">
          <Link href={videoItem.authLink}> <CheckCircleOutlined className="icon"/>{videoItem.auth}</Link>
        </div>
        <div className="view">
        <Space>
          <span>666K 次观看</span>
          <span><LikeOutlined/> 75%</span>
        </Space>
        </div>
      </div>
    </div>
